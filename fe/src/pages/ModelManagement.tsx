...
const ModelRow: React.FC<{ model: ModelSummary }> = ({ model }) => {
  const { data: status } = useModelStatus(model.modelId, model.status === 'TRAINING');
  ...
  return (
    <tr style={{ borderBottom: '1px solid #ddd' }}>
      <td style={{ padding: '12px' }}>{model.modelName}</td>
    ...
    <div style={{ padding: '20px' }}>
      ...
      {!models || models.length === 0 ? (
        <div style={{ 
          ...
          <p>No models found. Start by training your first model!</p>
        </div>
      ) : (
